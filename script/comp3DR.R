...
# Function to compute 3DR ratio with p-values.
comp3DR<-function(dat,assembly,threshold,dist,peak.GR=NULL,region.GR=NULL,testPalindromic=F){
 ...
 if(length(distpp)>20 & length(distpm)>20 & length(distmp)>20 & length(distmm)>20){
  ...
  names(RVec)=c("3DR","3DRlower95%","3DRupper95%","3DC","3DR.pvalue","3DC.pvalue",
	"Npairs","Nmotifs","MedianDistPM","MedianDistMP",
	"MedianDistPP","MedianDistMM","AssemblyLength","PalindromeValue")
 
  res=list(RVec=RVec,distanceMat=distanceMat)
  return(res)
 }else{
  return("Not enough motifs to compute 3DR.")
 }
}# END OF comp3DR()
