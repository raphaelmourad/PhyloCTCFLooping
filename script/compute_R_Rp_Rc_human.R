...
# This R script computes different R ratios from the human genome: 
# - the classic R when only motifs are provided; 
# - the Rp when predicted ChIP-seq peaks are provided;
# - the Rc when conservation scores are provided;
...
# Compute Rp for CTCF motifs + predicted CTCF peaks, Rp=1.44
...
CDFpred=compDistFun(data,assembly,threshold,dist,peak.GR=CTCFpred.GR)
print(CDFpred$RVec)

# Compute Rc for CTCF motifs + conservation, Rc=1.64
cons.GR=import.bed(paste0("data/CTCF_cons/",assembly,"_cons_50b.bed"))
cons.GR=cons.GR[cons.GR$score>quantile(cons.GR$score,0.7)] 
CDFcons=compDistFun(data,assembly,threshold,dist,peak.GR=cons.GR)
print(CDFcons$RVec)
