...
# Function to compute the null distribution of the difference of R values between two species
# This function uses permutations to compute the null distribution
compDiffRPerm<-function(ass1,ass2){
...
# Load precomputed R for different species
...
diffRexp=sapply(1:numPerm,function(x){compDiffRPerm(ass1,ass2)})
diffRobs=matPvalPM[matPvalPM[,1]==ass2,2]-matPvalPM[matPvalPM[,1]==ass1,2]
pvaldiffR=sum(abs(diffRexp)>abs(diffRobs))/length(diffRexp)
