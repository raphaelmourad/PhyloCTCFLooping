...
# This R script computes 3DR for different chromosome regions.
...
# Compute 3DR for different chromosome regions
assembly="hg38"
data=read.table(paste0("data/CTCF_motif/hg38.tsv"),sep='\t',header=T)[,-c(1:2)]
matPvalPM=NULL
for(i in 1:length(regionList)){
 CDFi=comp3DR(data,assembly,threshold,dist,region.GR=regionList[[i]],testPalindromic=F)
 if(length(CDFi)>1){
